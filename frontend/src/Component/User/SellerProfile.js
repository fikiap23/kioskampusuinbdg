...
const SellerProfile = () => {
    ...
    const fetchingSellerData = async () => {
      try {
        const response = await fetch(
          `https://kioskampusuinbdg-sigma.vercel.app/api/user/${id}`
        )
        if (!response.ok) {
        ...
        const sellerProductResponse = await fetch(
          `https://kioskampusuinbdg-sigma.vercel.app/api/products/${id}`
        )
  ...
  const whatsappLink = seller ? `https://wa.me/${seller.no_wa}` : '#'
