...
const CardMyProduct = ({ data }) => {
  ...
  const handleDelete = () => {
    ...
    if (confirmDelete) {
      fetch(
        `https://kioskampusuinbdg-sigma.vercel.app/api/product/${data.product_id}`,
        {
          method: 'DELETE',
        }
      )
  ...
  return (
    <div className="flex w-full items-center justify-between py-2 px-8 m-1 text-xs">
      ...
      <Link to={`/edit-product?id=${data.product_id}`}>
        <button className="text-center bg-orange-400 mr-16 ml-12 py-1 px-4 rounded-md text-white hover:bg-orange-500">
          Edit
        </button>
      </Link>
      ...
      <button
        ...
        className="text-center bg-red-400 py-1 px-4 mr-2 rounded-md text-white hover:bg-red-600"
      >
        Delete
      </button>
    </div>
